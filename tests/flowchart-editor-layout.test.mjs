...
test('流程图持久化前会校正文档会话，避免交互后退回思维导图', () => {
  ...
  assert.match(
    source,
    /ensureFlowchartDocumentSession\(\) \{[\s\S]*?documentMode:\s*'flowchart'[\s\S]*?isFullDataFile:\s*true/
  )
  assert.match(
    source,
    /persistFlowchartState\([\s\S]*?this\.ensureFlowchartDocumentSession\(\)[\s\S]*?markDocumentDirty\(dirty\)/
  )
})
...
test('流程图连接拖拽新建节点后保持连续扩展状态，不立即打断到文字编辑', () => {
  ...
  assert.match(flowchartEdgeLogicSource, /this\.ensureFlowchartEdge\(sourceNode\.id, newNode\.id\)/)
...
test('流程图支持商业泳道模板，编辑态、小地图和模板预览都会渲染责任分区', () => {
  ...
  assert.match(flowchartEditorSource, /flowchartSwimlane/)
  assert.match(flowchartEditorSource, /flowchartLanes\(\)/)
  assert.match(flowchartEditorSource, /getSwimlaneStyle\(/)
  assert.match(flowchartEditorSource, /:lanes="flowchartLanes"/)
  assert.match(flowchartMinimapSource, /flowchartMinimapLane/)
  assert.match(flowchartInspectorSource, /flowchartTemplateLane/)
...
test('流程图模板泳道不再自带固定颜色，而是统一跟随当前主题', () => {
  assert.match(flowchartDocumentSource, /accent = ''/)
  assert.match(flowchartDocumentSource, /lane\.accent \|\| theme\?\.accent \|\| '#2563eb'/)
  assert.match(flowchartInspectorSource, /getTemplateLaneStyle\(templateItem, lane\)/)
  assert.match(flowchartStyleSource, /\.flowchartMinimapLane/)
  assert.match(flowchartStyleSource, /\.flowchartMinimapEdge/)
  assert.match(flowchartStyleSource, /var\(--flowchart-accent\)/)
...
test('流程图白板模式提供连接手柄、连线中点工具条与行内编辑状态', () => {
  ...
  assert.match(flowchartEdgeLayerSource, /labels\.edgeEditText/)
  assert.match(flowchartEdgeLayerSource, /labels\.edgeInsertNode/)
  assert.match(flowchartEdgeLayerSource, /labels\.edgeDeleteLine/)
  assert.match(flowchartEdgeLayerSource, /flowchartEdgeToolbarText/)
  assert.match(flowchartSelectionSource, /getEdgeLabelToolbarPlacement = edge =>/)
  assert.match(flowchartSelectionSource, /placement:\s*getEdgeLabelToolbarPlacement\(edge\)/)
