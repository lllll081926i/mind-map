...
const loadFlowchartReconnectMethods = async () => {
  const source = flowchartReconnectSource
    .replace(
      /import\s*\{[\s\S]*?\}\s*from\s*'\.\/flowchartEditorShared'/,
      `const FLOWCHART_NODE_HIT_PADDING = 28;
      const FLOWCHART_STRAIGHT_EDGE_SNAP_THRESHOLD = 12;`
    )
    .replace(
      /import\s*\{[\s\S]*?\}\s*from\s*'@\/services\/flowchartDocument'/,
      `const normalizeFlowchartEdgeLabelPosition = value => value;
      const projectFlowchartPointToPolyline = () => ({ ratio: 0.5, point: { x: 0, y: 0 } });`
    )
  return import(
    `data:text/javascript;base64,${Buffer.from(source, 'utf8').toString('base64')}`
  )
...
test('流程图快速加节点条提供形状搜索和回车添加，吸收 draw.io 形状库查找体验', () => {
  assert.match(flowchartQuickAddBarSource, /flowchartQuickShapeSearch/)
  assert.match(flowchartQuickAddBarSource, /v-model="shapeQuery"/)
  assert.match(flowchartQuickAddBarSource, /filteredNodeTypes/)
  assert.match(flowchartQuickAddBarSource, /@keydown\.enter\.prevent="addFirstMatchedNode"/)
  assert.match(flowchartQuickAddBarSource, /@keydown\.esc="clearShapeSearch"/)
  assert.match(flowchartQuickAddBarSource, /shapeSearchPlaceholder/)
  assert.match(flowchartStyleSource, /\.flowchartQuickShapeSearch/)
  assert.match(langSource, /"shapeSearchPlaceholder": "搜索形状"/)
})
...
test('流程图外围工具保持画布优先，窄屏只横向滚动不挤压画布高度', () => {
  const compactToolbarMedia = flowchartStyleSource.match(
    /@media \(max-width: 1180px\) \{[\s\S]*?(?=@media \(max-width: 900px\))/
  )?.[0] || ''
  assert.match(flowchartStyleSource, /\.flowchartToolbar\s*\{[\s\S]*?overflow:\s*hidden/)
  assert.match(flowchartStyleSource, /\.flowchartToolbar\s*\{[\s\S]*?flex-wrap:\s*nowrap/)
  assert.match(flowchartStyleSource, /\.flowchartToolbarGroup\s*\{[\s\S]*?overflow-x:\s*auto/)
  assert.doesNotMatch(compactToolbarMedia, /\.flowchartToolbar\s*\{[\s\S]*?flex-wrap:\s*wrap/)
  assert.doesNotMatch(flowchartStyleSource, /padding-top:\s*156px/)
  assert.match(flowchartStyleSource, /\.flowchartQuickAddBar\s*\{[\s\S]*?max-width:\s*calc\(100vw - 24px\)/)
  assert.match(flowchartStyleSource, /\.flowchartQuickAddBar\s*\{[\s\S]*?overflow-x:\s*auto/)
  assert.match(flowchartStyleSource, /\.flowchartQuickShapeSearch\s*\{[\s\S]*?&:focus-within/)
})
...
test('流程图快速加节点在视口中心被占用时会寻找空位，避免叠住已有节点', async () => {
  const { flowchartNodeMethods } = await loadFlowchartNodeMethods()
  const methods = flowchartNodeMethods
  const context = {
    ...methods,
    selectedNodeIds: [],
    flowchartData: {
  ...
  const placementPoint = methods.getNodePlacementPoint.call(context, 'process')

  assert.ok(placementPoint.x > 900)
  assert.equal(placementPoint.y, 482)
})

test('流程图复制一份会沿用空位查找，避免复制结果叠到已有节点上', async () => {
  const { flowchartNodeMethods } = await loadFlowchartNodeMethods()
  const methods = flowchartNodeMethods
  const context = {
    ...methods,
    ...
    flowchartData: {
      nodes: [
        { id: 'source', type: 'process', text: '源节点', x: 0, y: 0, width: 168, height: 72 },
        { id: 'blocker', type: 'process', text: '占位节点', x: 48, y: 48, width: 168, height: 72 }
      ],
      edges: []
    },
    getSelectedNodes() {
  ...
  methods.duplicateSelectedNodes.call(context)

  const duplicatedNode = context.flowchartData.nodes.find(node => {
    return node.id !== 'source' && node.id !== 'blocker'
  })
  assert.ok(duplicatedNode)
  assert.notDeepEqual(
    { x: duplicatedNode.x, y: duplicatedNode.y },
    { x: 48, y: 48 }
  )
  const blocker = context.flowchartData.nodes.find(node => node.id === 'blocker')
  assert.equal(
    duplicatedNode.x + duplicatedNode.width > blocker.x &&
      duplicatedNode.x < blocker.x + blocker.width &&
      duplicatedNode.y + duplicatedNode.height > blocker.y &&
      duplicatedNode.y < blocker.y + blocker.height,
    false
  )
})
...
test('流程图节点拖动后的连线重算仍优先回归单折线，而不是保留多余手工折线', () => {
  assert.match(flowchartEditorSource, /relaxedLayout\.pathPoints\.length <= 3/)
  assert.match(flowchartEditorSource, /edge\.route = null/)
})
...
test('流程图手工折点写回 route 时会自动清理重复与共线冗余点', async () => {
  const { flowchartReconnectMethods } = await loadFlowchartReconnectMethods()
  const context = {
    ...flowchartReconnectMethods
  }
  const edge = {
    sourcePoint: { x: 288, y: 156 },
    targetPoint: { x: 420, y: 356 },
    route: null
  }

  context.applyManualRouteToEdge(edge, [
    { x: 420, y: 156 },
    { x: 420, y: 260 },
    { x: 420, y: 260 }
  ])
  ...
  assert.deepEqual(edge.route, {
    manualPoints: [{ x: 420, y: 156 }]
  })
  assert.match(flowchartReconnectSource, /simplifyEdgeManualRoutePoints\(/)
})
...
test('流程图支持拖拽重连已有连线端点，并把逻辑拆到独立重连模块', () => {
  ...
  assert.match(flowchartReconnectSource, /manualPoints/)
  assert.match(flowchartReconnectSource, /pathPoints\.slice\(1, -1\)/)
