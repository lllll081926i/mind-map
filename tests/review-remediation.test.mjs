...
const docSource = fs.readFileSync(path.resolve('src/pages/Doc.vue'), 'utf8')
...
test('运行时 store 改为惰性初始化，不在模块顶层直接创建实例', () => {
  ...
  assert.match(runtimeSource, /export const getPreferredMindMapThemeValue = expectDark =>/)
  assert.match(runtimeSource, /export const toggleThemeMode = \(\) =>/)
  assert.match(runtimeSource, /export const applyMindMapThemeMode = \(mindMap, nextIsDark\) =>/)
...
test('应用入口补充全局错误处理，根样式不再禁止所有文本选择', () => {
  ...
  assert.match(appSource, /id="app" :class="\{ isDark: isDark \}"/)
  assert.match(appSource, /document\.documentElement\.classList\.toggle\('app-dark-theme', !!value\)/)
  assert.match(appSource, /#app\.isDark/)
  assert.match(docSource, /useThemeStore/)
  assert.match(docSource, /:class="\{ isDark: isDark \}"/)
...
test('桌面文件系统目录白名单为空时必须拒绝访问而不是放行', () => {
  assert.doesNotMatch(tauriFsSource, /放行所有访问/)
  assert.doesNotMatch(
    tauriFsSource,
    /allowed_roots\.is_empty\(\)\s*\{[\s\S]*?return Ok\(\(\)\);[\s\S]*?\}/
  )
  assert.match(tauriFsSource, /没有可用的目录访问授权/)
})
